...
#include "ZlatkoTimer.h"

ZlatkoTimer::ZlatkoTimer() : timeInSeconds(0), tempTime(0), initialized(false) {
  rtc.begin();
  rtc.setY2kEpoch(0);
}

void ZlatkoTimer::reset() {
  timeInSeconds = 0;
  tempTime = 0;
  initialized = false;
}

void ZlatkoTimer::initTime() {
  if(! initialized) {
    rtc.setY2kEpoch(0);
    initialized = true;
  }
...
const int * ZlatkoTimer::getTime() {
  computeTime();
  return valueOfTime;
}

const unsigned int ZlatkoTimer::getTimeInSeconds() const {
  return timeInSeconds;
}

void ZlatkoTimer::incrementTime() {
  tempTime = rtc.getY2kEpoch();
}

unsigned int ZlatkoTimer::getTempTime() {
  return tempTime;
}
